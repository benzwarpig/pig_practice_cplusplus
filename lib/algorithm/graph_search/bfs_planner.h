...
class BfsSearchPlanner {
 public:
  ...
  void search(
      const std::function<bool(const Grid2D&, const Map&)> is_target,
      ...
      std::unordered_set<Grid2D, HashPoint, EqPoint>& search_area) {
    if (!is_accessible(start_point, map)) {
      spdlog::info("Start point in BFS is Unaccessible!!! ({}, {})",
    ...
    spdlog::info("Finish BFS, count {}, cost time {} ms", count,
                 duration.count());
  }
};
};  // namespace graph_search
