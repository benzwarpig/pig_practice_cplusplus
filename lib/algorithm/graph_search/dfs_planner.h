...
class DfsSearchPlanner {
 public:
  ...
  void search(FloodFillSelector selector, FloodFillSearcher searcher,
              ...
              const Grid2D &current_grid, const std::vector<Grid2D> &neighbours,
    ...
    while (!s_list.empty() && count < INT32_MAX) {
      ...
      // 5. 对多个最短距离下标打分，选出最优的格子
      if (candidate_index.size() == 0) {
      } else if (candidate_index.size() == 1) {
        is_select = true;
        select_grid = candidate_grid.at(
            candidate_grid_dis.at(candidate_index.at(0)).first);
      } else {
        ...
        for (int i = 0; i < candidate_index.size(); i++) {
          ...
          for (const auto neigh : neighbours) {
            ...
            if (!visited_list.count(tmp_neigh_grid) &&
              ...
              tmp_distance += current_grid.EuclideanDistance(tmp_neigh_grid);
            }
          }
