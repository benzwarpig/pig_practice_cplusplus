...
if draw_trajectory == 1:
    ...
    # 创建一个空的散点图对象
    scatter = plt.scatter([], [], color='green', marker='o', s=10)
    ...
    # 创建一个散点图对象用于突出显示第一个点
    highlight, = plt.plot([], [], 'ro', markersize=5)


    # 存储更新的点的最大数量
    max_points = 10000000  # 可以根据需要调整显示的点的数量
    ...
    def update(frame):
        step = 20
        start_index = max(0, frame - max_points + 1)
        current_points = trajectory[start_index:frame+step]
        line.set_data(current_points[:, 0], current_points[:, 1])
        if frame > 0:
            highlight.set_data(current_points[-1, 0], current_points[-1, 1])
        return line, highlight

    # 创建动画
    ani = FuncAnimation(plt.gcf(), update, frames=np.arange(0, len(trajectory), 20), interval=50, blit=True, repeat=False)
